...
struct ProductCarouselView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack {
                HStack {
                    Button(action: {
                        print("navigation to image selection is \($navigateToImageSelection)")
                    ...
                    ZStack {
                        ForEach(products.indices, id: \.self) { index in
                            VStack {
                                ...
                                Spacer()
                                VStack {
                                    Text(String(products[index].website))
                                        .font(.callout)
                                        .padding()
    ...
    func cardScale(for index: Int, propotion: CGFloat = 0.2) -> CGFloat {
        ...
        if index >= activeCardIndex {
            let progress = min(abs(dragOffset) / (screenWidth / 2), 1)
            return 1 - propotion * CGFloat(adjustedIndex) + (dragOffset < 0 ? propotion * progress : -propotion * progress)
        }
        return 1
    }
}
